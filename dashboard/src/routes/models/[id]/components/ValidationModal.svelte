<script lang="ts">
	...
	async function triggerBackgroundValidation(validationId: string, metadata: any, uploadedFolder: any) {
		...
		try {
			...
			// Call server route (server uses VALIDATOR_URL_INTERNAL)
			console.log('📊 Running full model validation in background via /api/validations/folder');
			const fd = new FormData();
			fd.append('validationName', `Background Validation ${new Date().toISOString()}`);
			fd.append('userName', 'system');
			fd.append('date', new Date().toISOString());
			fd.append('datasetName', uploadedFolder?.data?.name || 'dataset');
			fd.append('folderName', uploadedFolder?.data?.name || 'dataset');
			fd.append('datasetDescription', '');
			fd.append('datasetCharacteristics', '');
			fd.append('metricsDescription', '');
			fd.append('performanceMetrics', '');
			fd.append('modelId', modelId || '');

			if (uploadedFolder.metadata) {
				fd.append('metadataFile', uploadedFolder.metadata);
			} else {
				const metadataBlob = new Blob([JSON.stringify(metadata)], { type: 'application/json' });
				fd.append('metadataFile', new File([metadataBlob], 'metadata.json', { type: 'application/json' }));
			}
			fd.append('dataFile', uploadedFolder.data);
			if (uploadedFolder.columnMetadata) {
				fd.append('columnMetadataFile', uploadedFolder.columnMetadata);
			}

			const serverValidationResponse = await fetch('/api/validations/folder', {
				method: 'POST',
				body: fd
			});

			const serverValidationResult = await serverValidationResponse.json();
			if (!serverValidationResponse.ok || !serverValidationResult?.success) {
				throw new Error(serverValidationResult?.error?.errors?.[0]?.message || serverValidationResult?.error || 'Server-side validation failed');
			}

			const validationResult =
				serverValidationResult?.validation?.validation_result ??
				serverValidationResult?.validation_result;
			...
			// Preserve old behavior/variables
	...
	async function handleSubmit() {
		...
		try {
			...
			// If creating new validation, trigger background validation
			if ($validationStore.mode === 'create' && result.validation) {
				const validationId = result.validation?.val_id || result.validation?.id;
				if (validationId) {
					console.log('🚀 Triggering background validation for val_id:', validationId);
					...
					triggerBackgroundValidation(validationId.toString(), modelMetadata, formData.uploadedFolder).catch(error => {
