...
const makeBody = (delimiter: string) => {
  ...
  const call = async (delimiter: string) => {
    const res = await fetch(`${INTERNAL_VALIDATOR_BASE}/validate-csv/`, {
      method: 'POST',
      body: makeBody(delimiter)
    });
  ...
  if (first.res.ok) return JSON.parse(first.raw);

  const delimiterErrorText =
    `${first.raw} ${first.parsed?.detail ?? ''} ${first.parsed?.message ?? ''}`;

  if (
    (first.res.status === 400 || first.res.status === 500) &&
    delimiterErrorText.includes('Could not determine delimiter')
  ) {
    const retry = await call(';');
    if (retry.res.ok) return JSON.parse(retry.raw;

    throw new ValidationError({
      code: 'VALIDATION_FAILED',
      message: 'CSV validation request failed',
      technicalDetails: `POST ${INTERNAL_VALIDATOR_BASE}/validate-csv/ -> ${first.res.status}: ${first.raw} | retry ';' -> ${retry.res.status}: ${retry.raw}`
    }, retry.res.status);
  }
  ...
  throw new ValidationError({
    code: 'VALIDATION_FAILED',
    message: 'CSV validation request failed',
    technicalDetails: `POST ${INTERNAL_VALIDATOR_BASE}/validate-csv/ -> ${first.res.status}: ${first.raw}`
  }, first.res.status);
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    return json({
      success: true,
      validation: result[0],
      message: 'Folder validation created successfully'
    });
