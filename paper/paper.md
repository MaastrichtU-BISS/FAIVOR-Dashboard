...
![FAIVOR system architecture showing the Dashboard (SvelteKit frontend with PostgreSQL) and ML-Validator (FastAPI backend) components, connected to FAIRmodels server and Docker-containerized AI models.](docs/techstack.excalidraw.png){#fig:architecture}

## Validation Backend (FAIVOR-ML-Validator)

The Python-based backend, built with FastAPI, provides a REST API for executing containerized AI models and computing standardized validation metrics (Figure 2). Key capabilities include:

![Validation engine architecture detailing the Docker handler for model execution, metrics computation modules, data statistics processing, and REST API endpoints.](FAIVOR-ML-Validator/architecture.drawio.png){#fig:validator}
...
![Database schema showing relationships between model checkpoints, validations, data statistics, and users.](docs/db-schema.drawio.png){#fig:database}

# Clinical Use Cases

FAIVOR has been developed and validated in collaboration with Maastricht University and Maastro Clinic for two primary use cases:

## Proton Therapy Treatment Selection (ProTRAIT)

AI models predicting treatment outcomes (e.g., dysphagia dose-response models for head and neck cancer patients) are used to determine eligibility for proton versus photon radiotherapy [@kalendralis2022independent]. These legally mandated models require continuous validation to ensure reliable treatment recommendations. FAIVOR enables:

- Pre-implementation validation when extending eligibility criteria to new cancer types
- Regular performance monitoring for deployed decision support models
- Evidence collection for MDR compliance documentation

## Deep Learning Auto-Contouring

Commercial and research deep learning models for automatic organ and tumor contouring on medical images require validation before clinical deployment and continuous monitoring thereafter [@lustberg2017clinical]. FAIVOR supports validation metrics specific to image segmentation (Dice scores, Hausdorff distances) and tracks performance as imaging equipment and protocols evolve.
...
The platform extends the FAIR4AI initiative developed in the NWA RAISE project, creating a web of linked data describing AI models and their independent validation results. This enables researchers to discover validation evidence, notified bodies to assess certification applications, and hospitals to find models validated in similar clinical contexts.
...
This work was supported by the Netherlands eScience Center under project NLESC.OEC.2023.072. We thank Maastro Clinic for clinical requirements and validation support, and the broader FAIR4AI and Health-RI communities for valuable feedback. FAIVOR builds upon foundational work from the NWA RAISE project (FAIR4AI work package) and the ProTRAIT consortium.
